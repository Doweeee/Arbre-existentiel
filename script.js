...
var SMALL_BUBBLE_NODES = { boite:1, inexp:1, perroquet:1, debrancher:1, nature:1 };
/* Nodes that use the extra-narrow (xs) bubble — 30% narrower than sm */
var XS_BUBBLE_NODES = { risques:1 };
...
function onImgLoad(){
  if(treeLoaded && rootsLoaded){
    ...
    fitView();
    requestRender();
    /* Animation d'entrée : trait base → ia → 3 racines */
    startAnim('intro');
  }
}
...
var BRANCH_VIEWS = {
  risques: { cx: 0.44*IMG_W, cy: 0.25*IMG_H, scale: 0.52 },
...
/* ════════════════════════════════════════════════════════════════════════
   ANIMATIONS DE TRAITS ORANGE
   ────────────────────────────────────────────────────────────────────────
   Chaque animation est définie par des segments à tracer dans l'ordre, en
   coordonnées image (IMG_W × IMG_H). On stocke une liste de "segments
   actifs" avec leur progression individuelle [0..1].

   STRUCTURE :
     - lineColor       : '#F7931D'
     - lineWidth       : épaisseur des traits (en px écran)
     - active          : tableau de segments en cours
        chaque segment = { from:{x,y}, to:{x,y}, t:0..1, speed:0.025, delay:ms }
   ════════════════════════════════════════════════════════════════════════ */
...
var ANIM = {
  ...
  lineWidth   : 2.5,
  active      : [],
  startedFor  : {}, /* {parentId : true} pour ne pas relancer 2 fois */
  running     : false
};

/* Convertit rx/ry (normalisés 1066×600) → coords image (IMG_W × IMG_H) */
function nodeImg(n){
  return { x: n.rx * IMG_W, y: n.ry * IMG_H };
}

/* Trouve le point le plus proche de P sur le segment [A, B] (projection
   orthogonale clampée). Utilisé pour le cas spécial Risques (intersection
   du Y inversé). */
function closestPointOnSegment(p, a, b){
  var abx=b.x-a.x, aby=b.y-a.y;
  var apx=p.x-a.x, apy=p.y-a.y;
  var ab2=abx*abx+aby*aby;
  if(ab2<1e-6) return {x:a.x, y:a.y};
  var t=(apx*abx+apy*aby)/ab2;
  t=Math.max(0, Math.min(1, t));
  return {x:a.x+t*abx, y:a.y+t*aby};
}

/* Construit les segments pour une "vague" d'animation depuis un parent vers
   ses enfants. Cas spécial : si parent='risques', on construit le Y inversé
   pour Survie + Mécanisme via le point d'intersection. */
function buildChildAnimSegments(parentId){
  var parent = NODES[parentId];
  if(!parent || parent.rx==null) return [];
  var pPos   = nodeImg(parent);
  var children = NODES_DATA.filter(function(n){ return n.pid===parentId && n.rx!=null; });
  var segs = [];

  if(parentId === 'risques'){
    /* Cas spécial — Y inversé.
       1. Trace principal : Risques → Degré de trop (en passant ~par l'inter)
       2. Intersection : point le plus proche de pInter sur ce trait
       3. Embranchements : intersection → Survie, intersection → Mécanisme
       4. Traits simples : Risques → Kasparov, Risques → Optimiseur          */
    var deg = NODES.degretrop, sur = NODES.survie, mec = NODES.mecact;
    var kas = NODES.kasparov,  opt = NODES.optimizer;
    if(deg && deg.rx!=null){
      var pDeg = nodeImg(deg);
      /* Point intermédiaire fourni par l'utilisateur (normalisé 1066×600) */
      var pInterRaw = { x: 0.4745 * IMG_W, y: 0.3121 * IMG_H };
      var pInter    = closestPointOnSegment(pInterRaw, pPos, pDeg);
      /* Trait principal Risques → Degré de trop, en 2 sous-segments
         pour respecter l'enchaînement temporel (avant l'intersection,
         puis après).                                                       */
      var delay=0;
      segs.push({ from:pPos,   to:pInter, t:0, speed:0.040, delay:delay });
      delay += 240;
      /* Depuis l'intersection : 3 ramifications en parallèle (Degré de trop,
         Survie, Mécanisme) — elles partent toutes simultanément.           */
      segs.push({ from:pInter, to:pDeg,   t:0, speed:0.045, delay:delay });
      if(sur && sur.rx!=null) segs.push({ from:pInter, to:nodeImg(sur), t:0, speed:0.050, delay:delay });
      if(mec && mec.rx!=null) segs.push({ from:pInter, to:nodeImg(mec), t:0, speed:0.050, delay:delay });
      /* Et les 2 autres traits droits depuis Risques (Kasparov, Optimiseur) */
      if(kas && kas.rx!=null) segs.push({ from:pPos, to:nodeImg(kas), t:0, speed:0.045, delay:0 });
      if(opt && opt.rx!=null) segs.push({ from:pPos, to:nodeImg(opt), t:0, speed:0.045, delay:0 });
    }
  } else {
    /* Cas général : traits droits parent → chaque enfant, démarrés ensemble */
    children.forEach(function(c){
      segs.push({ from:pPos, to:nodeImg(c), t:0, speed:0.045, delay:0 });
    });
  }
  /* Horodatage des delays (timestamp + ms) */
  var t0 = performance.now();
  segs.forEach(function(s){ s.startAt = t0 + (s.delay||0); });
  return segs;
}

/* Animation d'entrée : depuis "base" → "ia" → 3 racines (idees/risques/nature) */
function buildIntroAnimSegments(){
  var base = NODES.base, ia = NODES.ia;
  if(!ia || ia.rx==null) return [];
  var segs = [];
  var t0   = performance.now();
  var delay= 600; /* léger temps d'attente après chargement */
  /* Si "base" est placé, on commence par base → ia */
  if(base && base.rx!=null){
    segs.push({ from:nodeImg(base), to:nodeImg(ia), t:0, speed:0.035, startAt:t0+delay, delay:delay });
    delay += 700;
  }
  /* Puis ia → 3 racines, parties simultanément */
  ['idees','risques','nature'].forEach(function(rid){
    var r = NODES[rid];
    if(r && r.rx!=null){
      segs.push({ from:nodeImg(ia), to:nodeImg(r), t:0, speed:0.040, startAt:t0+delay, delay:delay });
    }
  });
  return segs;
}

/* Démarrer une animation (parentId ou 'intro') si pas déjà jouée */
function startAnim(key){
  if(ANIM.startedFor[key]) return;
  ANIM.startedFor[key] = true;
  var segs;
  if(key === 'intro') segs = buildIntroAnimSegments();
  else                segs = buildChildAnimSegments(key);
  if(segs.length === 0) return;
  ANIM.active = ANIM.active.concat(segs);
  if(!ANIM.running){
    ANIM.running = true;
...
function animStep(){
  ...
  var anyMoving = false;
  for(var i=0; i<ANIM.active.length; i++){
    var s = ANIM.active[i];
    if(now < s.startAt) { anyMoving = true; continue; }
    if(s.t < 1){ s.t = Math.min(1, s.t + s.speed); anyMoving = true; }
  }
  requestRender();
  if(anyMoving){ requestAnimationFrame(animStep); }
  else        { ANIM.running = false; }
}

/* Convertit un point (coords image) vers les coords écran via la même
   transformation que les nœuds (drawImage du fond) */
function imgToScreen(p){
  /* viewCX/CY = centre vue dans l'espace étendu (image coords) */
  var sw = W / viewScale;
  var sh = H / viewScale;
  var vx0 = viewCX - sw/2;
  var vy0 = viewCY - sh/2;
  return { x:(p.x - vx0) * viewScale, y:(p.y - vy0) * viewScale };
}

/* Dessine les traits d'animation actifs (par-dessus le fond, sous les nœuds) */
function drawAnimLines(){
  if(ANIM.active.length === 0) return;
  ctx.save();
  ctx.strokeStyle = ANIM.lineColor;
  ctx.lineWidth   = ANIM.lineWidth;
  ctx.lineCap     = 'round';
  ctx.shadowColor = ANIM.lineColor;
  ctx.shadowBlur  = 8;
  for(var i=0; i<ANIM.active.length; i++){
    var s = ANIM.active[i];
    if(s.t <= 0) continue;
    var a = imgToScreen(s.from);
    var b = imgToScreen(s.to);
    var bx = a.x + (b.x - a.x) * s.t;
    var by = a.y + (b.y - a.y) * s.t;
    ctx.beginPath();
    ctx.moveTo(a.x, a.y);
    ctx.lineTo(bx, by);
    ctx.stroke();
...
function openBubble(n){
  ...
  else if(XS_BUBBLE_NODES[n.id]){ bblI.src=''; bblI.style.display='none'; size='xs'; }
  else       { bblI.src=''; bblI.style.display='none'; size=SMALL_BUBBLE_NODES[n.id]?'sm':'md'; }
  bbl.className='bbl vis '+size;
  var bw=size==='lg'?500:size==='md'?540:size==='sm'?360:252;
...
window.addEventListener('mouseup', function(e){
  ...
  if(hitId){
    ...
    if(ZOOM_TRIGGERS[hitId]){
      ...
      /* Lancer l'animation des traits orange parent → enfants
         (la 1re fois seulement, le flag startedFor empêche le redéclenchement) */
      startAnim(hitId);
    }
    openBubble(n);
